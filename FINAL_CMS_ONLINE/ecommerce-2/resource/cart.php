  ...
  if(isset($_GET['add'])) {
    ...
    while($row = fetch_array($query)) {
      ...
      if($row['product_quantity'] != $_SESSION['product_'. $_GET['add']]) {
        ...
        set_message("Nous avons seulement " . $row['product_quantity'] . " " . "{$row['product_title']}" . "  disponible(s)");
        redirect("../public/checkout.php");
...
<tr>
  ...
  <img width="80" src='../resource/images/{$row['product_image']}'>

  </td>
  <td>GNF {$row['product_price']}</td>
  <td>{$value}</td>
  <td>GNF {$sub}</td>
  <td><a class='btn btn-warning' href="../resource/cart.php?remove={$row['product_id']}"><span class='glyphicon glyphicon-minus'></span></a>  
   <a class='btn btn-success' href="../resource/cart.php?add={$row['product_id']}"><span class='glyphicon glyphicon-plus'></span></a>
