 ...
 <?php 
  if (isset($_POST['submit'])) {
    ...
    if (verification($utilisateur,$email)) {
       $message=' 
       Les informations que vous avez saisies existent déjà, réessayez d\'autres!<a  href="registration.php" style="color:black"><br><i class="fa fa-arrow-circle-right">Rétour</i></a>';
    }
    else if
    ...
    $query_registration="INSERT INTO users(username,user_password,user_email,user_role,user_status,message)
     VALUES('$utilisateur','$password','$email','user','valide','Message envoyé')";
                ...
                <h1 class="text-center btn-info"><?php echo _REGISTER; ?></h1>
                    <form role="form" action="" method="post" id="login-form" autocomplete="off" class="well">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="utilisateur" id="username" class="form-control" placeholder="<?php echo _USERNAME; ?>" required>
                        </div>
                         <div class="form-group">
                            <label for="email" class="sr-only">Email</label>
                            <input type="email" name="email" id="email" class="form-control" placeholder="<?php echo _EMAIL; ?> Example:goundo@example.com" required>
                        </div>
                         <div class="form-group">
                            <label for="password" class="sr-only">Mot de Pass</label>
                            <input type="password" name="password" id="myInput" class="form-control" placeholder="<?php echo _PASSWORD; ?>" required>
                 ...
                 <?php else:?>
                 <h3 class="text-center btn-info"><?php echo "$message"; ?></h3>
                  <?php endif;?>
            </div> <!-- /.col-xs-12 -->
        </div> <!-- /.row -->
    </div> <!-- /.container -->
