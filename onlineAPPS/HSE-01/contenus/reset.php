    ...
    if(isset($_POST['password']) && isset($_POST['confirmPassword'])){
        ...
        if($_POST['password'] === $_POST['confirmPassword']){
            ...
            if($stmt = mysqli_prepare($connect, "UPDATE users SET  pass='{$hashedPassword}' WHERE user = ?")){
                ...
                mysqli_stmt_execute($stmt);
                $compte_lien=escape($_GET['compte']);
                $query_lien= "SELECT token FROM users WHERE user= '$compte_lien'" ;
                ...
                if( $row['token']!=escape($_GET['token'])){
                 echo'<script>
        alert("Ce lien n\'est plus valable! \n\ Faites une autre demande de modification de mot de pass,\n\ en cliquant sur RETOUR puis MOT DE PASS OUBLIE?");
        </script>';
            }
             else {
          if(mysqli_stmt_affected_rows($stmt) >= 1){
            flash('modif','Modification faite avec succès');

                  set_message("<button><a href='index.php'>connectez-vous maintenant! </a></button>");
                  $stmt = mysqli_prepare($connect, "UPDATE users SET token='' WHERE user = ?");
                   mysqli_stmt_bind_param($stmt, "s", $_GET['compte']);
                mysqli_stmt_execute($stmt);
         ...
         echo'<script>
        alert("Tapez un même mot de pass dans les deux champs,merci de réessayer encore");
        </script>';
    }
                               ...
                               <h2 class="text-center">Réinitialisation?</h2>
                                ...
                                <div class="panel-body">
                                 ...
                                 <?php else:?>
                                     ...
                                     <h2 class=text-center style="color: red"> <?php display_message();?></h2>
                                <?php endif;?>
                                    <h2>UMS-HSE</h2>
