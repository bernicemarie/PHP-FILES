    ...
    if (verifie($user)) {
        if ($stmt=mysqli_prepare($connect,"UPDATE users SET token='$token' WHERE user=?")) {
           ...
           mysqli_stmt_close($stmt);
                    ...
                    <a href="https://hse.ums-ap.com/HSE/contenus/reset.php?compte='.$user.'&token='.$token.'"> 
                     <button class="btn btn-lg btn-info btn-block">Réinitialisez votre mot de pass ici</button> </a><br>
                    Si vous n\'êtes pas l\'auteur de cette demande de réinitialisation du mot de pass,prière de supprimer ce message.
                    </p>';
                    if($mail->send()){
                        $emailSent = true;
         ...
         echo'<script>
        alert("Votre identifiant n\'est pas valide, merci de réessayer encore");
        </script>';
    }
}
                        ...
                        <div class="text-center">
                           ...
                           <?php if (!isset($emailSent)):?>
                                ...
                                <div class="panel-body">
                                   ...
                                   <?php else:?>
                                    <h2>Un message vous sera envoyé dans moins d'une minute, merci de vérifier votre boîte aux lettres!</h2>
                                <?php endif;?>
                        </div>
                    </div>
                </div>
